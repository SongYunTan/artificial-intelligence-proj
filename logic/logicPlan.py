...
def sentence3() -> Expr:
    ...
    uno = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0)|(~PacmanAlive_0 & PacmanBorn_0))
    dos = ~(PacmanAlive_0 & PacmanBorn_0)
    tres = PacmanBorn_0
    return conjoin([uno,dos,tres])
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # prev = atLeastOne(possible_causes)
    # final = now % prev
    # return final
    ...
    # return conjoin([PropSymbolExpr(pacman_str, x, y, time=now) % disjoin([moved_causes_sent])] + auxilary_expression_definitions)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for i in range(len(all_coords)):
        if i not in non_outer_wall_coords: A = True
        else: A = False

        pacphysics_sentences.append(A>>(~B&~D))
