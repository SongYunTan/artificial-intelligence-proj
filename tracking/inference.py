...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for joinVariable in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
            currentFactorsList.append(joinedFactor)
            if joinedFactor == '1':  # check only one unconditioned variable
                continue
            fullJoint = joinFactors(currentFactorsList)
            eliminate(fullJoint, joinVariable)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        sum = self.total()
        sampleNo = random.random()

        for key in self.keys():
            sampleNo -= self[key]/sum
            if sampleNo <0:
                return key
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None and ghostPosition == jailPosition:
            # ghost is caught and in jail with probability 1
            return 1.0
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            # for each position, get probability distribution of ghost's position at time t + 1, 
            # given that the ghost is at position pos at time t
            distribution[pos] = self.getPositionDistribution(gameState, pos)

        for pos in self.allPositions:
            for prevpos in self.beliefs:
                newbeliefs[pos] += distribution[prevpos][pos] * self.beliefs[prevpos]
